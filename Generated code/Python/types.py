...
class WebhookInfo:
    ...
    attribute max_connections: Maximum allowed number of simultaneous HTTPS connections to the webhook for update delivery
...
class Video:
    ...
    attribute file_name: Original filename as defined by sender
    attribute mime_type: Mime type of a file as defined by sender
    attribute file_size: File size in bytes"""
...
class Location:
    ...
    attribute proximity_alert_radius: Maximum distance for proximity alerts about approaching another chat member, in meters. For sent live locations only."""
...
class InlineKeyboardButton:
    ...
    attribute switch_inline_query_current_chat: If set, pressing the button will insert the bot's username and the specified inline query in the current chat's input field. Can be empty, in which case only the bot's username will be inserted.This offers a quick way for the user to open your bot in inline mode in the same chat – good for selecting something from multiple options.
...
class CallbackQuery:
    ...
    attribute data: Data associated with the callback button. Be aware that the message, which originated the query, can contain no callback buttons with this data.
...
class ChatInviteLink:
    ...
    attribute member_limit: Maximum number of users that can be members of the chat simultaneously after joining the chat via this invite link; 1-99999
...
class InputMediaVideo:
    ...
    attribute media: File to send. Pass a file_id to send a file that exists on the Telegram servers (recommended), pass an HTTP URL for Telegram to get a file from the Internet, or pass “attach://<file_attach_name>” to upload a new one using multipart/form-data under <file_attach_name> name. More info on Sending Files »
    attribute thumb: Thumbnail of the file sent; can be ignored if thumbnail generation for the file is supported server-side. The thumbnail should be in JPEG format and less than 200 kB in size. A thumbnail's width and height should not exceed 320. Ignored if the file is not uploaded using multipart/form-data. Thumbnails can't be reused and can be only uploaded as a new file, so you can pass “attach://<file_attach_name>” if the thumbnail was uploaded using multipart/form-data under <file_attach_name>. More info on Sending Files »
...
class InlineQueryResultMpeg4Gif:
    ...
    attribute id: Unique identifier for this result, 1-64 bytes
    attribute mpeg4_url: A valid URL for the MP4 file. File size must not exceed 1MB
...
class InlineQueryResultCachedMpeg4Gif:
    ...
    attribute type: Type of the result, must be mpeg4_gif
    attribute id: Unique identifier for this result, 1-64 bytes
    attribute mpeg4_file_id: A valid file identifier for the MP4 file
    attribute title: Title for the result
    attribute caption: Caption of the MPEG-4 file to be sent, 0-1024 characters after entities parsing
    attribute parse_mode: Mode for parsing entities in the caption. See formatting options for more details.
    attribute caption_entities: List of special entities that appear in the caption, which can be specified instead of parse_mode
...
class InputInvoiceMessageContent:
    ...
    attribute photo_url: URL of the product photo for the invoice. Can be a photo of the goods or a marketing image for a service. People like it better when they see what they are paying for.
    attribute photo_size: Photo size
    ...
    attribute send_phone_number_to_provider: Pass True, if user's phone number should be sent to provider
    attribute send_email_to_provider: Pass True, if user's email address should be sent to provider
...
class ShippingAddress:
    ...
    attribute country_code: ISO 3166-1 alpha-2 country code
...
class SuccessfulPayment:
    ...
    attribute order_info: Order info provided by the user
